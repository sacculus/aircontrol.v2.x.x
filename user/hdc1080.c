...
h_hdc1080 ICACHE_FLASH_ATTR hdc1080_setup(
		h_brzo_i2c_bus i2c_bus,
		uint8_t i2c_address,
		uint16_t i2c_frequency,
		uint16_t i2c_ack_timeout)
...
bool ICACHE_FLASH_ATTR hdc1080_get_reg(
		h_hdc1080 sensor,
		uint8_t reg,
		uint16_t *pData)
{
...
bool ICACHE_FLASH_ATTR hdc1080_set_reg(
		h_hdc1080 sensor,
		uint8_t reg,
		uint16_t data)
{
...
bool ICACHE_FLASH_ATTR hdc1080_set_config(h_hdc1080 sensor, uint16_t data)
{
	...
	bool result = hdc1080_set_reg(sensor,
			HDC1080_REG_CONFIGURATION,
			(data & HDC1080_REG_CONFIG_WRITE_MASK));
	return result;
}

bool ICACHE_FLASH_ATTR hdc1080_get_temperature(
		h_hdc1080 sensor,
		float *p_data)
{
	bool result = false;
    ...
    if (brzo_i2c_end_transaction(s->i2c_bus) == 0)
    {
    	os_printf("[hdc1080_get_temperature] Send measure command successful\n");
    	/*
