		...
		uint32_t clk_stretch)
{
	...
	if ((bus = os_malloc(sizeof(i2c_bus_t))) != NULL)
	{
		...
		if (system_get_cpu_freq() == 160) {
			...
			if (clk_stretch < 100) bus->clk_stretch = 730;
			else bus->clk_stretch = 730 * clk_stretch / 100;
		}
		else {
			// 80 MHz
			bus->iteration_remove_spike = 7;
			if (clk_stretch < 100) bus->clk_stretch = 470;
			else bus->clk_stretch = 470 * clk_stretch / 100;
		}
...
i2c_device_t * ICACHE_FLASH_ATTR brzo_i2c_setup_device(
		uint8_t address,
		uint16_t frequency)
{
	...
	if ((device = os_malloc(sizeof(i2c_device_t))) != NULL) {
		...
		if (system_get_cpu_freq() == 160) {
			if (fr_sel <= 1) device->iteration_scl_halfcycle = 156;
			else if (fr_sel == 2) device->iteration_scl_halfcycle = 79;
			else if (fr_sel == 3) device->iteration_scl_halfcycle = 51;
			else if (fr_sel == 4) device->iteration_scl_halfcycle = 38;
			else if (fr_sel == 5) device->iteration_scl_halfcycle = 30;
			else if (fr_sel == 6) device->iteration_scl_halfcycle = 24;
			else if (fr_sel == 7) device->iteration_scl_halfcycle = 20;
			else if (fr_sel == 8) device->iteration_scl_halfcycle = 18;
			else if (fr_sel == 9) device->iteration_scl_halfcycle = 15;
			else device->iteration_scl_halfcycle = 14;
		}
		else {
			// 80 MHz
			if (fr_sel <= 1) device->iteration_scl_halfcycle = 80;
			else if (fr_sel == 2) device->iteration_scl_halfcycle = 37;
			else if (fr_sel == 3) device->iteration_scl_halfcycle = 26;
			else if (fr_sel == 4) device->iteration_scl_halfcycle = 19;
			else if (fr_sel == 5) device->iteration_scl_halfcycle = 14;
			else if (fr_sel == 6) device->iteration_scl_halfcycle = 11;
			else if (fr_sel == 7) device->iteration_scl_halfcycle = 9;
			else device->iteration_scl_halfcycle = 8;
		}
		return device;
	}
	return NULL;
...
void ICACHE_FLASH_ATTR brzo_i2c_start_transaction(
		h_brzo_i2c_bus bus,
		uint8_t address,
		uint16_t frequency)
{
...
uint8_t ICACHE_FLASH_ATTR brzo_i2c_end_transaction(h_brzo_i2c_bus bus)
{
	 ...
	 * Bus not initialized properly, nothing to do
	 */
	if (bus == NULL) return I2C_ERR_INIT_ERROR;
...
void ICACHE_FLASH_ATTR brzo_i2c_write(
		h_brzo_i2c_bus bus,
		uint8_t *p_data,
		uint32_t length,
		bool repeated)
...
void ICACHE_FLASH_ATTR brzo_i2c_read(
		h_brzo_i2c_bus bus,
		uint8_t *p_data,
		uint32_t length,
		bool repeated)
...
void ICACHE_FLASH_ATTR brzo_i2c_ack_polling(
		h_brzo_i2c_bus bus,
		uint16_t timeout)
{
	/*
	...
	if (timeout == 0) {
		...
		iteration_ACK_polling_timeout =
				timeout / p_bus->active_device->ack_polling_loop_usec;
	}
